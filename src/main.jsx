...
const journey = [
  ...
  [
    "04",
    "MONITOR",
    "Monitor your progress with appropriate professional guidance.",
  ],
];
...
const faqs = [
  [
    "What is Feloklear®?",
    "Feloklear® Capsule is an AfrOganiks formulation containing three botanical ingredients traditionally used in herbal health preparations.",
  ],
  ...
  [
    "How should I take Feloklear®?",
    "2 capsules 10 minutes before breakfast and 2 capsules 10 minutes before dinner.",
  ],
  ["What should I take Feloklear® with?", "Warm or normal water."],
  [
    "Can I take it with soft drinks?",
    "No. The supplied document says not to take it with soft drinks or other carbonated drinks.",
  ],
  [
    "Can pregnant women use Feloklear®?",
    "No. Pregnant women should not use Feloklear®.",
  ],
  [
    "Can lactating mothers use Feloklear®?",
    "No. Lactating mothers should not use Feloklear®.",
  ],
  [
    "Does Feloklear® guarantee pregnancy?",
    "No. FELOKLEAR® is not a promise of a pregnancy.",
  ],
  ["What is the NAFDAC registration number?", "A7-2459L."],
  [
    "How much does the 60-day protocol cost?",
    "₦50,000 including consultation and shipping.",
  ],
  [
    "Can I speak to a consultant?",
    "Yes. Use the consultant CTA to ask questions before ordering.",
  ],
];
...
const orderFields = [
  ...
  {
    id: "notes",
    label: "Delivery notes or questions for the consultant",
    type: "textarea",
  },
];
...
const ctaStyles = {
  primary:
    "border border-fern bg-fern text-white shadow-button hover:border-moss hover:bg-moss hover:text-white",
  secondary:
    "border border-moss/25 bg-white text-fern hover:border-moss hover:bg-linen hover:text-fern",
  light:
    "border border-white bg-white text-fern shadow-button hover:border-linen hover:bg-linen hover:text-fern",
  ghostLight:
    "border border-white/45 bg-transparent text-white hover:border-white hover:bg-white/10 hover:text-white",
};
...
function ProductMockup({ compact = false }) {
  return (
    <div
      className={`relative mx-auto ${compact ? "max-w-xs" : "max-w-xl"}`}
      aria-label="Replaceable FELOKLEAR product mockup"
    >
      ...
      <div className="relative rounded-[2rem] border border-white/70 bg-gradient-to-br from-white to-linen p-6 shadow-soft">
        <div className="grid items-center gap-5 sm:grid-cols-[0.85fr_1.15fr]">
          <div className="mx-auto flex min-h-[17rem] w-full max-w-[12.5rem] flex-col rounded-2xl bg-fern p-4 text-white shadow-2xl sm:min-h-[18rem]">
            ...
            <div className="mt-8 text-center text-xs uppercase tracking-[0.32em] text-linen">
              AfrOganiks
            </div>
            ...
            <div className="mt-3 text-center text-sm text-linen/85">
              Capsule 500 mg
            </div>
          ...
          <div className="space-y-3">
            {ingredients.map((item) => (
              <div
                key={item.name}
                className="flex items-center justify-between rounded-2xl border border-moss/10 bg-white/80 p-4"
              >
                <div>
                  <p className="font-serif text-lg text-fern">{item.name}</p>
                  <p className="text-sm text-bark">{item.note}</p>
                </div>
                <span className="rounded-full bg-linen px-3 py-1 text-sm font-bold text-moss">
                  {item.dose}
                </span>
              </div>
            ))}
        ...
        <div className="hidden items-center gap-7 lg:flex">
          {navLinks.map(([label, href]) => (
            <a
              ...
              className="text-sm font-medium text-bark transition hover:text-fern"
            >
              {label}
            </a>
          ))}
          ...
          <div className="grid gap-2">
            {navLinks.map(([label, href]) => (
              <a
                ...
                className="rounded-xl px-3 py-3 text-sm font-semibold text-bark hover:bg-linen"
              >
                {label}
              </a>
            ))}
            <CTAButton className="mt-2 w-full">Order — ₦50,000</CTAButton>
            <CTAButton className="w-full" href="/chat" variant="secondary">
              Chat
            </CTAButton>
          </div>
        </div>
...
function Section({ id, eyebrow, title, children, className = "" }) {
  return (
    <section
      id={id}
      className={`px-4 py-16 sm:px-6 sm:py-20 lg:px-8 ${className}`}
    >
      <div className="mx-auto max-w-7xl">
        {(eyebrow || title) && (
          <Reveal className="mx-auto mb-10 max-w-3xl text-center">
            ...
            {title && (
              <h2 className="font-serif text-3xl font-semibold leading-tight text-fern sm:text-5xl">
                {title}
              </h2>
            )}
          </Reveal>
...
function Hero() {
  return (
    <section
      id="top"
      className="relative overflow-hidden bg-linen px-4 py-12 sm:px-6 lg:px-8 lg:py-20"
    >
      <div className="mx-auto grid max-w-7xl items-center gap-12 lg:grid-cols-[1fr_0.9fr]">
        <Reveal>
          <p className="mb-5 text-xs font-bold uppercase tracking-[0.18em] text-rose">
            Women's Reproductive & Vaginal Health
          </p>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-bark">
            Hearing words like "blocked tubes," "hydrosalpinx," "PID," or "tubal
            factor infertility" can make the fertility journey feel frightening
            and uncertain.
          </p>
          <p className="mt-4 max-w-2xl text-xl font-medium leading-8 text-ink">
            A diagnosis is not the end of your fertility journey. Understanding
            the problem is the beginning of taking action.
          </p>
          <div className="mt-8 rounded-3xl border border-moss/10 bg-white/70 p-6">
            ...
            <p className="mt-3 leading-7 text-bark">
              ...
              vaginal health, including concerns associated with
              infection-related tubal blockage and hydrosalpinx.
            </p>
          </div>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <CTAButton className="w-full sm:w-auto">
              Get Feloklear Now <ArrowRight size={16} />
            </CTAButton>
            <CTAButton
  ...
  return (
    <div className="min-h-screen bg-porcelain text-ink">
      ...
      <main>
          ...
          <div className="grid items-center gap-8 lg:grid-cols-2">
            <Reveal className="space-y-5">
              <p className="text-lg leading-8 text-bark">
                ...
                When tubes are damaged or obstructed, conception can become more
                difficult.
              </p>
              <div className="rounded-3xl border border-moss/15 bg-white p-6">
                <BookOpenCheck className="mb-4 text-rose" />
                <p className="font-serif text-2xl text-fern">
                  Tubal disease is a significant contributor to female-factor
                  infertility.
                </p>
                <p className="mt-3 text-bark">
                  The supplied material attributes to ASRM that tubal disease
                  accounts for approximately 25-35% of female-factor
                  infertility.
                </p>
              </div>
            </Reveal>
        ...
        <Section title="What is tubal blockage?">
          <div className="grid gap-6 lg:grid-cols-3">
            {["Narrowed", "Damaged", "Obstructed"].map((item) => (
              <Reveal
                key={item}
                className="rounded-3xl border border-moss/10 bg-white p-7 shadow-soft"
              >
                <Leaf className="mb-5 text-sage" />
                <h3 className="font-serif text-2xl text-fern">{item}</h3>
                <p className="mt-3 text-bark">
                  A fallopian tube may become {item.toLowerCase()} due to
                  factors that need proper evaluation.
                </p>
              </Reveal>
            ))}
          </div>
          <Reveal className="mt-8 rounded-[2rem] bg-fern p-8 text-white sm:p-10">
            <p className="text-lg leading-8 text-linen">
              Possible causes mentioned in the supplied material include
              previous pelvic infections, pelvic inflammatory disease,
              endometriosis, and previous pelvic surgery. Hydrosalpinx is a
              condition in which a fallopian tube becomes blocked and filled
              with fluid.
            </p>
            <p className="mt-5 font-serif text-2xl">
              Not every suspected blockage is the same.
            </p>
            <p className="mt-3 text-linen/90">
              HSG or sonohysterography are commonly used to evaluate tubal
              patency, according to the supplied ASRM guidance. Before you begin
              any fertility protocol: know what you are dealing with.
            </p>
          </Reveal>
        </Section>
        ...
        <Section
          id="ingredients"
          eyebrow="Introducing FELOKLEAR®"
          title="A formulation created with women's reproductive health in mind."
          className="bg-linen"
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {concerns.map((concern, index) => (
              <Reveal
                key={concern}
                delay={index * 0.03}
                className="rounded-3xl border border-moss/10 bg-white p-6 shadow-soft"
              >
                ...
                <p className="mt-3 text-sm leading-6 text-bark">
                  Presented as an area of support or concern, not a guaranteed
                  medical cure.
                </p>
              </Reveal>
            ))}
        ...
        <Section
          title="Don't guess. Understand your reproductive health."
          className="bg-linen"
        >
        ...
        <Section
          id="protocol"
          eyebrow="The Recommended Course"
          title="The FELOKLEAR® 60-Day Protocol"
        >
          <Reveal className="rounded-[2rem] bg-fern p-6 text-white shadow-soft sm:p-10">
            <div className="grid gap-5 lg:grid-cols-3">
              <ProtocolItem
                title="Morning"
                main="2 Capsules"
                text="10 minutes before breakfast"
              />
              <ProtocolItem
                title="Evening"
                main="2 Capsules"
                text="10 minutes before dinner"
              />
              <ProtocolItem
                title="Duration"
                main="60 Days"
                text="4 packs for two months"
              />
            </div>
            <div className="mt-8 grid gap-4 rounded-3xl bg-white/10 p-5 sm:grid-cols-2">
              <p>
                <strong>Take with warm or normal water.</strong>
              </p>
              <p>
                <strong>
        ...
        <Section title="Why a 60-day approach?" className="bg-linen">
          <Reveal className="mx-auto max-w-4xl text-center">
            <p className="text-xl leading-9 text-bark">
              Reproductive health does not change overnight. A structured
              protocol encourages consistency rather than taking a product for a
              few days, stopping, changing products and starting again.
            </p>
            <p className="mt-7 font-serif text-4xl text-fern">
              CONSISTENCY MATTERS.
            </p>
            <p className="mt-4 text-bark">
              This does not imply that 60 days guarantees a medical result.
            </p>
          </Reveal>
        </Section>
        ...
        <Section
          id="safety"
          title="FELOKLEAR® IS NOT A PROMISE OF A PREGNANCY."
          className="bg-fern text-white"
        >
          <Reveal className="mx-auto max-w-5xl">
            <p className="text-center text-2xl font-medium text-linen">
              It is a step toward taking your reproductive health seriously.
            </p>
            <p className="mt-6 text-center leading-8 text-linen/90">
              Blocked tubes can have different causes and degrees of severity.
              Some forms of tubal disease may require procedures or assisted
              reproductive treatment. Treatment decisions can depend on site and
              extent of disease, age, ovarian reserve, other infertility
              factors, and individual reproductive goals.
            </p>
            <div className="mt-8 grid gap-4 sm:grid-cols-5">
              {[
                "Get properly evaluated.",
                "Understand your diagnosis.",
                "Take your reproductive health seriously.",
                "Use products responsibly.",
                "Monitor your progress with appropriate professional guidance.",
              ].map((item) => (
                <div
                  key={item}
                  className="rounded-2xl bg-white/10 p-4 text-sm font-semibold text-linen"
                >
                  {item}
                </div>
              ))}
        ...
        <Section title="Who is Feloklear® for?">
          <Checklist
            items={[
              "Infection-associated reproductive-health concerns",
              "Vaginal discharge or odour",
              "Menstrual discomfort",
              "Infection-related tubal-health concerns",
              "Hydrosalpinx-related concerns",
        ...
        <Section title="Important medication warning">
          <Reveal className="rounded-[2rem] border border-rose/25 bg-rose/10 p-8">
            ...
            <p className="mt-4 font-semibold text-fern">
              Visitors with questions should speak with a qualified healthcare
              professional.
            </p>
          </Reveal>
        </Section>
        ...
        <Section title="What about side effects?" className="bg-linen">
          <Reveal className="mx-auto max-w-4xl rounded-[2rem] bg-white p-8 shadow-soft">
            ...
            <Checklist
              items={["Vomiting", "Heartburn", "Increased flatulence"]}
              compact
            />
            <p className="mt-5 font-semibold text-fern">
              If an unexpected or severe reaction occurs: discontinue use and
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_0.8fr]">
              <div>
                ...
                <div className="mt-7 rounded-3xl bg-linen p-5">
                  {ingredients.map((item) => (
                    <p key={item.name} className="text-sm leading-7 text-bark">
                      {item.name} — {item.dose}
                    </p>
                  ))}
                </div>
              ...
              <div className="rounded-[1.5rem] bg-fern p-7 text-white">
                <p className="text-sm uppercase tracking-[0.18em] text-linen">
                  2-month protocol + consultation + shipping
                </p>
  ...
  return (
    <div className="min-h-screen bg-linen text-ink">
      ...
      <main className="px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto grid min-w-0 max-w-7xl gap-8 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
          <Reveal className="min-w-0 lg:sticky lg:top-24 lg:self-start">
            <a
              href="/#offer"
              className="mb-6 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.08em] text-fern hover:text-rose"
            >
              <ArrowRight className="rotate-180" size={16} /> Back to product
              page
            </a>
            <div className="rounded-[2rem] bg-fern p-6 text-white shadow-soft sm:p-8">
              ...
              <div className="mt-8 rounded-3xl bg-white/10 p-5">
                <p className="text-sm uppercase tracking-[0.16em] text-linen/80">
                  Package total
                </p>
                <p className="mt-3 font-serif text-6xl">₦50,000</p>
                <p className="mt-2 text-linen/85">
                  2-month protocol + consultation + shipping
                </p>
              </div>
          ...
          <Reveal className="min-w-0">
            <div className="min-w-0 rounded-[2rem] bg-white p-5 shadow-soft sm:p-8">
              {submitted ? (
                <div className="py-8 text-center">
                  <HeartHandshake
                    className="mx-auto mb-5 text-sage"
                    size={42}
                  />
                  <h2 className="font-serif text-4xl text-fern">
                    Your order details have been captured.
                  </h2>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mt-8 rounded-3xl border border-rose/25 bg-rose/10 p-5">
                    <div className="flex gap-3">
                      ...
                      <div>
                        <h3 className="font-serif text-2xl text-fern">
                    ...
                    <label className="mt-5 flex cursor-pointer gap-3 rounded-2xl bg-white p-4">
                      ...
                      <span className="text-sm font-medium leading-6 text-bark">
                        I confirm I have read the pregnancy, lactation,
                        medication, and side-effect warnings and want a
                        consultant to confirm my order.
                      </span>
                    </label>
                  </div>
...
function FormField({ field }) {
  ...
  return (
    <label className={`min-w-0 ${isWide ? "sm:col-span-2" : ""}`}>
...
function ChatPage() {
  ...
  return (
    <div className="min-h-screen bg-linen text-ink">
      <Navbar />
      <main className="px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto grid min-w-0 max-w-7xl gap-8 lg:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)]">
          <Reveal className="min-w-0">
            <a
              href="/"
              className="mb-6 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.08em] text-fern hover:text-rose"
            ...
            <div className="rounded-[2rem] bg-fern p-6 text-white shadow-soft sm:p-8">
              ...
              <p className="mt-5 leading-8 text-linen/90">
                ...
                warnings, delivery, and next steps before ordering.
              </p>
          ...
          <Reveal className="min-w-0">
            <section
              className="min-w-0 overflow-hidden rounded-[2rem] bg-white shadow-soft"
              aria-label="FELOKLEAR consultant chat box"
            >
              <div className="flex min-w-0 items-center justify-between gap-3 border-b border-moss/10 bg-porcelain p-4 sm:gap-4 sm:p-5">
              ...
              <div className="flex min-h-[28rem] flex-col bg-gradient-to-b from-white to-linen/70">
                ...
                <form
                  ...
                  className="min-w-0 border-t border-moss/10 bg-white p-4"
                >
                  <label htmlFor="chat-message" className="sr-only">
                    Message for FELOKLEAR consultant
                  </label>
                  ...
                  <div className="grid min-w-0 gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
                    <textarea
                      ...
                      className="max-h-36 min-h-12 min-w-0 w-full resize-y rounded-2xl border border-moss/15 bg-porcelain px-4 py-3 text-base text-ink outline-none transition placeholder:text-bark/45 focus:border-rose focus:ring-4 focus:ring-rose/10"
                    />
                    <button
                      type="submit"
                      className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-fern bg-fern px-5 py-3 text-sm font-bold uppercase tracking-[0.08em] text-white shadow-button transition hover:border-moss hover:bg-moss sm:px-6"
                    >
                      Send <ArrowRight size={16} />
                    </button>
                  </div>
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    <CTAButton href="/order" className="w-full">
                      Order instead
                    </CTAButton>
                    <CTAButton
                      href="#"
...
function Checklist({ items, compact = false }) {
  return (
    <Reveal
      className={`grid gap-3 ${compact ? "mt-5" : "mx-auto max-w-4xl sm:grid-cols-2"}`}
    >
      {items.map((item) => (
        <div
          key={item}
      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[1fr_1fr_1fr]">
        ...
        <div className="grid gap-2 text-sm">
          {navLinks.map(([label, href]) => (
            <a
              key={label}
              href={`${homePrefix}${href}`}
              className="text-linen/75 hover:text-white"
            >
              {label}
            </a>
          ))}
        ...
        <div>
          <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
            ...
            <CTAButton href="/chat" variant="ghostLight">
              Chat with a Consultant
            </CTAButton>
          </div>
...
function MobileStickyCTA() {
  return (
    <div className="mobile-sticky-cta fixed inset-x-0 bottom-0 z-50 border-t border-moss/10 bg-porcelain/95 p-3 shadow-2xl backdrop-blur lg:hidden">
